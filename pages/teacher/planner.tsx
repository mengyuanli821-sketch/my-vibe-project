...
export default function AIClassPlannerPage() {
  ...
  async function generatePlan() {
    ...
    try {
      const response = await fetch("/api/class-planner", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ student_ids: studentIds, theme, difficulty: Number(difficulty), duration: Number(duration), new_students: Number(newStudents), props, intention }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? "Could not generate plan");
      setPlan(data.plan); setSource(data.source === "ai" ? "ai" : "adaptive"); setExpandedPose(0);
  ...
  return (
    <Layout title="AI Class Planner" action={<Link className="premium-button inline-flex" href="/teacher">← Teacher Studio</Link>}>
      ...
      <div className="teacher-tabs"><Link href="/teacher">Capability profile</Link><Link className="teacher-tab-active" href="/teacher/planner">AI Class Planner</Link></div>
      ...
      <div className="planner-workspace mt-6">
        <section className="planner-form-card">
          ...
          <button className="mt-6 w-full rounded-full bg-[#294a3c] px-5 py-3 text-sm font-semibold text-white shadow-lg hover:bg-[#1d3329] disabled:opacity-50" disabled={isGenerating || !studentIds.length} onClick={generatePlan} type="button">{isGenerating ? "Reading history & shaping sequence…" : "Generate class plan →"}</button>
        </section>
